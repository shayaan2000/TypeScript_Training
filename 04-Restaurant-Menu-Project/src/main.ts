...
// Category Tabs - these are generated dynamically based on data on initialization
interface ICategoryTab {
  id: number;
  title: string;
}
let categoryTabs: ICategoryTab[];

//============== Selecting Elememts =================
const categoryFilterBarEl = document.querySelector(
  ".filter-bar"
) as HTMLUListElement;
const categoryTabsEl = document.getElementsByClassName(
  "filter-tab"
) as HTMLCollection;
const menuSectionEl = document.querySelector(".menu-section") as HTMLDivElement;

//=============== Generating Markups ================
// Generating an item markup
const generateMenuItemMarkup = (item: IMenuItem): string => {
  return `
    <div class="item">
...
const generateCategoryTabMarkup = (categoryTab: ICategoryTab): string => {
  return `
...
const generateCategoryTabs = (): void => {
  ...
  categoryTabs.push(
    ...uniqueCategories.map((categoryTitle, i) => {
...
const displayResults = (): void => {
  ...
  const currentItemsMarkup: string = currentItems
    .map((item: IMenuItem) => generateMenuItemMarkup(item))
    .join("");
...
const renderTabs = (): void => {
  // generating markups
  const categoryTabsMarkups: string = categoryTabs
...
const setTabActive = (tabId: number): void => {
...
const filterResults = (filterCategory: string): void => {
  currentItems = allItems.filter(
    ({ category }) => filterCategory.toLowerCase() === category.toLowerCase()
  );
};
...
const handleFilter = (event: Event): void => {
  ...
  if (!target.closest(".filter-tab")) return;
  ...
  // Checking for 'All'
  tabId === 0
    ? (currentItems = allItems.map((item) => ({ ...item })))
